...
public class Main {
	...
	public static void main(String[] args){
		...
		Thread t = new Thread(new Runnable(){
			public void run(){
				try (
					DatagramSocket server = new DatagramSocket(PORT) //Création de la connexion côté serveur, en spécifiant un port d'écoute
				){
					...
					while(true)
					{
						...
						print("Reçu de la part de " + packet.getAddress() + " sur le port " + packet.getPort() + " : ");	//Récuperation du contenu et affichage dans la console
						println(str);																						//
						
						
						packet.setLength(buffer.length); //réinitialisation la taille du datagramme pour les futures réceptions
